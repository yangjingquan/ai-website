...
Vue Router configuration lives in `src/router/index.js`.
...
const router = createRouter({ history: createWebHistory(), routes, scrollBehavior(to, from, savedPosition) { if (savedPosition) return savedPosition; if (to.hash) return { el: to.hash, behavior: 'smooth', top: 16 }; return { top: 0 } } })
router.afterEach((to) => { document.title = to.meta.title || '小肥羊开发工作室 - 商业系统定制开发' })
export default router
```

`/` is the main acquisition page; the other four routes are capability and process detail pages with the same shell.
